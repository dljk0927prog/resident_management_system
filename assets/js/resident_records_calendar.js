...
async function checkAuth() {
    try {
        ...
        const infoEl = document.getElementById('residentHeaderInfo');
        if (infoEl) {
            infoEl.textContent = data.resident.name
...
async function loadCalendar() {
    ...
    try {
        const response = await fetch('api/get_resident_records_calendar.php?month=' + encodeURIComponent(ym));
        const data = await response.json();
        if (!data.success) {
...
async function loadDayRecords() {
    ...
    const url = 'api/get_resident_records.php?date=' + encodeURIComponent(selectedDate) + '&limit=200&page=1' + (kw ? '&q=' + encodeURIComponent(kw) : '');
    try {
        ...
        const unknown = typeof t === 'function' ? t('unknownVisitorIP') : '未知';
        tbody.innerHTML = data.records.map(r => `
            <tr>
                ...
                <td>${escapeHtml(r.visitor_ip || unknown)}</td>

...
try {
    ...
    $countSql = "SELECT COUNT(*) as total FROM scan_records sr" . $whereSql;
    ...
    try {
        $stmt = $conn->prepare("
            ...
            LEFT JOIN qr_codes qr ON sr.qr_code_id = qr.id
            $whereSql
        ...
        // 表未升级时使用旧字段查询，并从 qr_codes 关联出单元楼、门牌号、日期、手机号（若 qr_codes 有这些列）
        try {
            $stmt = $conn->prepare("
